package com.everyday.api.global.jwt.filter;
...
public class JwtFilterAuthenticationTest {
    ...
    @BeforeEach
    private void init() {
        memberRepository.save(Member.builder().username(USERNAME).password(delegatingPasswordEncoder.encode(PASSWORD)).name("Member1").email("email1@abc.com").role(Role.USER).build());
        clear();
    }
